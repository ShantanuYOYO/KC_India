...
<div class='metric-card'>
  ...
  <div class='metric-value'>{value}</div>
            ...
            st.markdown("<div class='chart-container'>", unsafe_allow_html=True)
            st.markdown(
                "<div class='chart-title'>&#9672; INITIAL QTY &middot; BALANCE &middot; DAMAGE QTY BY BRAND</div>",
                unsafe_allow_html=True
            )
            ...
            brand_comp = (
                ...
                .reset_index()
                .sort_values('INITIAL_QTY', ascending=False)
            )
            ...
            if not brand_comp.empty:
                ...
                fig_bc.add_trace(go.Bar(
                    name='Initial Qty',
                    x=brand_comp['BRAND'],
                    y=brand_comp['INITIAL_QTY'],
                    text=brand_comp['INITIAL_QTY'].apply(lambda v: f"{v:,.0f}"),
                ...
                fig_bc.add_trace(go.Bar(
                    name='Balance Qty',
                    x=brand_comp['BRAND'],
                    y=brand_comp['BALANCE'],
                    text=brand_comp['BALANCE'].apply(lambda v: f"{v:,.0f}"),
                ...
                fig_bc.add_trace(go.Bar(
                    name='Damage Qty',
                    x=brand_comp['BRAND'],
                    y=brand_comp['DAMAGE_PRODUCTS'],
                    text=brand_comp['DAMAGE_PRODUCTS'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#E76F51", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig_bc.update_layout(
                    barmode='group',
                    title="Initial Qty vs Balance vs Damage by Brand",
                    showlegend=True,
                    legend=dict(
                ...
                fig_bc = _dark_layout(
                    fig_bc, "Brand", "Quantity",
                    extra_xaxis={'categoryorder': 'array',
                                 'categoryarray': brand_comp['BRAND'].tolist()},
                    height=520
                )
            ...
            st.markdown("<div class='chart-container'>", unsafe_allow_html=True)
            st.markdown(
                "<div class='chart-title'>&#9672; INITIAL QTY &middot; BALANCE &middot; DAMAGE QTY BY SEASON</div>",
                unsafe_allow_html=True
            )
            ...
            if not season_comp.empty:
                fig_sc = go.Figure()
                fig_sc.add_trace(go.Bar(
                    name='Initial Qty',
                    x=season_comp['SEASON'],
                    y=season_comp['INITIAL_QTY'],
                    text=season_comp['INITIAL_QTY'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#D4AF37", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig_sc.add_trace(go.Bar(
                    name='Balance Qty',
                    x=season_comp['SEASON'],
                    y=season_comp['BALANCE'],
                    text=season_comp['BALANCE'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#E67E22", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig_sc.add_trace(go.Bar(
                    name='Damage Qty',
                    x=season_comp['SEASON'],
                    y=season_comp['DAMAGE_PRODUCTS'],
                    text=season_comp['DAMAGE_PRODUCTS'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#E76F51", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig_sc.update_layout(
                    barmode='group',
                    title="Initial Qty vs Balance vs Damage by Season",
                    showlegend=True,
                    legend=dict(
                        font=dict(color=TEXT_COLOR, size=11),
                        bgcolor='rgba(0,0,0,0)',
                ...
                fig_sc = _dark_layout(
                    fig_sc, "Season", "Quantity",
                    extra_xaxis={'categoryorder': 'array',
                                 'categoryarray': season_comp['SEASON'].tolist()},
                    height=520
                )
